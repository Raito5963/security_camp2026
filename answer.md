...
そもそも、私はこの講義で登場するCI/CDパイプラインについて全く理解していないので、少し調べました。
...
苦労した点は共有機能です。もともと、ユーザ登録などなく、サイトに入ったら誰かが追加した単語帳をそのまま使える、自分が追加した単語帳をそのまま誰かが使ってくれる状態でした。しかし、よくよく考えてみるとWebというのはインターネットの世界にあるため入ろうと思えば不特定多数の人がこのサイトに入ることも可能です。そうなると、自分たちの情報をなんの障壁もなくさらしている状態であるためセキュリティの面で非常に危険でした。そこで、Supabase Authを利用してメールアドレスとパスワードを登録することでユーザ登録を行えることができ、ユーザごとに表示されるCSVを切り替えられるようにしました。ユーザ登録とユーザごとにDBから単語帳を抽出して表示するという機能はすぐにできましたが、問題は共有でした。
...
ほかにも様々なWebアプリを開発しました。

3. [AI予定帳](https://planner.aokiju.com) (Command Planner)<br />
...
私自身が利用した経験はありませんが、前述のチーム開発にてT氏がK8sを利用しておりました。
