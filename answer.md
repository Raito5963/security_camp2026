...
#### 概要
`CI/CD`とは、継続的インテグレーションと継続的デリバリー(継続的デプロイ)を略したもの。

- 継続的インテグレーション<br/>
コードの安定性を損なうことなく、複数の開発者が同時にコード作業とマージを行える。
- 継続的デリバリー<br/>
承認を得たうえでソフトウェアをテスト、検証、準備をするための自動化されたワークフロー。
- 継続的デプロイ<br>
継続的デリバリーに似ているが、承認が事前設定されたルールを基に自動化される点が異なる。

CI/CDが組み合わさると、手動で行われていたプロセスを自動化するパイプラインとして実装できる。
...
> 参考文献<br />
> [CI/CD パイプラインとは](https://aws.amazon.com/jp/what-is/ci-cd-pipeline/)
...
この講義で行われる開発ライフサイクルの保護に非常に興味があります。また、コンテナ環境やCI/CDパイプラインに関しては全く知識がないので新しい知識をプロから直接学べることにも期待しています。
...
> [Why typescript](https://www.typescriptlang.org/ja/why-create-typescript/)
> [バックエンド開発でPythonを習得するメリットとは?就活に向けて効率的に学ぶ方法](https://rookie.levtech.jp/guide/detail/60015/)
> [Rust](https://rust-lang.org/ja/)
> [Javaでなにができるの？メリット・デメリットなど初心者の方でもわかりやすく紹介!](https://portal.dymcareer.jp/column/engineer/java#point3)
...
> [Kubernetes](https://kubernetes.io/docs/concepts/overview/#why-you-need-kubernetes-and-what-can-it-do)
